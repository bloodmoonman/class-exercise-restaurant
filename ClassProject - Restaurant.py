...
class Menu:
  ...
  def __repr__(self):
    return "{name} menu is available between {st} and {et}.".format(name=self.name, st=self.start_time, et=self.end_time)
...
early_bird_menu = Menu("Early Bird Dinner", early_bird_items, 15, 18)
...
price = brunch_menu.calculate_bill(["pancakes", "home fries", "coffee"])
print(price)

price2 = early_bird_menu.calculate_bill(["salumeria plate", "mushroom ravioli (vegan)"])
print(price2)
...
business_name = Business("Basta Fazoolin with my Heart", [flagship_store, new_installment])
